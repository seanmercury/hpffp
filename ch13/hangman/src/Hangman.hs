...
instance Show Puzzle where
  show (Puzzle word discovered guessed) =
    ...
    ++ " Just use this in develop: " ++ word

freshPuzzle :: String -> Puzzle
freshPuzzle a = Puzzle a (map (const Nothing) a) []

charInWord :: Puzzle -> Char -> Bool
charInWord (Puzzle s _ _) a = elem a s

alreadyGuessed :: Puzzle -> Char -> Bool
alreadyGuessed (Puzzle _ _ g) a = elem a g
...
-- 13.14 Chap Exercise
fillInCharacter (Puzzle word filledInSoFar s) c = Puzzle word newFilledInSoFar (if elem (Just c) newFilledInSoFar then s else (c : s))
  where 
    zipper guessed wordChar guessChar = if wordChar == guessed
                                        then Just wordChar
                                        else guessChar
